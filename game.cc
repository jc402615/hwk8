...
//temp
	...
	getline(cin, answer);
    //check for lower case "pass"
    if(answer == "pass"){
        answer = "PASS";
    }
    //let user enter lower or upper case i.e. a6 or A6
    else if(answer[0] >= 'a' && answer[0] <= 'h'){
        answer[0] -= ('a' - 'A');
    }
	return answer;
    }
	...
	// value of the best of these moves from the opponent's perspective.
    	compute_moves(moves);
	// assert(!moves.empty( ));
    //if there are no moves available, give the options for the comp to PASS
        if(moves.empty()){
            moves.push("PASS");
        }
    	best_value = INT_MIN;
	...
	compute_moves(moves);
	//assert(!moves.empty( ));
    if(moves.empty()){ //the computer has no available moves
        make_move("PASS"); //this is the only possible move for the comp to make
        cout << "It is the computer's turn: " << endl;
        cout << "Computer's move: " << "PASS" << endl;
        return;
    }
    ...
    {
        //Phase 3 - to differentiate players
    ...
    void game::change_move_number(int adjustment){
        move_number += adjustment;
    }
}
